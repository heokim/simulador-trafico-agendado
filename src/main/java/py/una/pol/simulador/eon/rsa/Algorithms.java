...
public class Algorithms {
    ...
    private static Graph<Integer, Link> getGrafoPonderadoPorEntropia(Graph<Integer, Link> grafoOriginal) {
        ...
        for (Link link : grafoOriginal.edgeSet()) {
            ...
            double pesoUso = calcularEntropiaDeEnlace(link);
    ...
    /**
     * Calcula la entropía total de un enlace.
     * Suma las transiciones de todos los núcleos del enlace.
     */
    public static int calcularEntropiaDeEnlace(Link link) {
        if (link == null || link.getCores() == null) {
            return 0;
        }

        int totalEntropy = 0;
        for (Core core : link.getCores()) {
            totalEntropy += calcularEntropiaPorCore(core);
        }
        return totalEntropy + 1;
    }

    /**
     * Calcula la entropía de un núcleo específico.
     * Recorre los Frequency Slots y cuenta los cambios de estado.
     */
