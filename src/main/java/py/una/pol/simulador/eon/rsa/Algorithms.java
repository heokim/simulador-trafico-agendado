...
public class Algorithms {
    ...
    /**
     * Ordena una lista de rutas candidatas (KSP) basándose en su uso actual,
     * utilizando una política
     * de "Least Loaded" (Menos Cargada).
     * <p>
     * El "peso" de cada ruta se calcula como la suma de los pesos de sus enlaces
     * individuales.
     * El peso de un enlace se define por el núcleo que tiene la <b>menor cantidad
     * de ranuras de frecuencia (FS) ocupadas</b>.
     * Es decir, para cada enlace, se busca el núcleo más libre (con menos FS
     * ocupados) y ese valor de ocupación
     * se suma al peso total de la ruta.
     * </p>
     * <p>
     * Las rutas se ordenan de forma ascendente: las rutas con menor peso (más
     * libres) aparecerán primero.
     * Este ordenamiento modifica la lista original proporcionada.
     * </p>
     *
     * @param kspPaths Lista de rutas candidatas (GraphPath) que será ordenada
     *                 in-place.
     */
    ...
    private static AllocationResult tryAllocatePath(GraphPath<Integer, Link> path, int fsIndex, Demand demand,
        ...
        for (Link link : links) {
            ...
            // Obtener núcleos ordenados (Estrategia: Entropy)
            List<Integer> coresList = getSortedCoresByEntropy(link);
